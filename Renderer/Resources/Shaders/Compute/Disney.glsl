...
vec3 DisneySample(State state, vec3 V, vec3 N, out vec3 L, out float pdf)
{
	...
	vec3 Csheen, Cspec0;
	float F0;
	TintColors(state.material, state.eta, F0, Csheen, Cspec0);

	// Model weights
	float dielectricWt = (1.0 - state.material.metallic) * (1.0 - state.material.specTrans);
	float metalWt = state.material.metallic;
	float glassWt = (1.0 - state.material.metallic) * state.material.specTrans;

	// Lobe probabilities
	float schlickWt = SchlickWeight(V.z);

	float diffPr = dielectricWt * Luminance(state.material.baseColor);
	float dielectricPr = dielectricWt * Luminance(mix(Cspec0, vec3(1.0), schlickWt));
	float metalPr = metalWt * Luminance(mix(state.material.baseColor, vec3(1.0), schlickWt));
	float glassPr = glassWt;
	float clearCtPr = 0.25 * state.material.clearcoat;
	...
	{
		vec3 H = SampleGGXVNDF(V, state.material.ax, state.material.ay, r1, r2);
	...
	{
		vec3 H = SampleGTR1(state.material.clearcoatRoughness, r1, r2);
...
vec3 DisneyEval(State state, vec3 V, vec3 N, vec3 L, out float pdf)
{
	...
	{
		f += EvalDisneyDiffuse(state.material, Csheen, V, L, H, tmpPdf) * dielectricWt;
		pdf += tmpPdf * diffPr;
	}
	...
	if (dielectricPr > 0.0 && reflect)
	{
		...
		float F = (DielectricFresnel(VDotH, 1.0 / state.material.ior) - F0) / (1.0 - F0);

		f += EvalMicrofacetReflection(state.material, V, L, H, mix(Cspec0, vec3(1.0), F), tmpPdf) * dielectricWt;
	...
	if (metalPr > 0.0 && reflect)
	{
		...
		vec3 F = mix(state.material.baseColor, vec3(1.0), SchlickWeight(VDotH));

		f += EvalMicrofacetReflection(state.material, V, L, H, F, tmpPdf) * metalWt;
		pdf += tmpPdf * metalPr;
	}
	...
	if (glassPr > 0.0)
	{
		...
		if (reflect)
		{
			f += EvalMicrofacetReflection(state.material, V, L, H, vec3(F), tmpPdf) * glassWt;
			pdf += tmpPdf * glassPr * F;
		}
		else
		{
			f += EvalMicrofacetRefraction(state.material, state.eta, V, L, H, vec3(F), tmpPdf) * glassWt;
	...
	{
		f += EvalClearcoat(state.material, V, L, H, tmpPdf) * 0.25 * state.material.clearcoat;
